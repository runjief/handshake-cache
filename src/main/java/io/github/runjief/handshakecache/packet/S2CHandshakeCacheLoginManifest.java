...
public class S2CHandshakeCacheLoginManifest extends HandshakeCacheLoginPacket {

    private final byte[][] tags;

    public S2CHandshakeCacheLoginManifest(byte[][] tags) {
        this.tags = tags;
    }
    ...
    public static S2CHandshakeCacheLoginManifest decode(FriendlyByteBuf buf) {
        int len = buf.readVarInt();
        byte[][] tags = new byte[len][];
        for (int i = 0; i < len; i++) {
            tags[i] = buf.readByteArray();
        }
        return new S2CHandshakeCacheLoginManifest(tags);
    }
